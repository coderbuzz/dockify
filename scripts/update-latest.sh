...
if [ "$MODE" = "docker" ]; then
  ...
  echo ""
  echo -e "${GREEN}Dockify updated${NC}"
...
echo "Downloading latest Dockify (no API call)..."
curl -fsSL -o "$BINARY" \
...
if ! "$BINARY" version >/dev/null 2>&1; then
  echo -e "${RED}Error: new binary validation failed. Keeping current version.${NC}"
  exit 1
fi
...
echo ""
echo -e "${GREEN}Dockify updated to $NEW_VERSION${NC}"
if [ "$MODE" = "binary+caddy" ]; then
