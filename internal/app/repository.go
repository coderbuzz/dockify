...
func (r *Repository) List() ([]App, error) {
	rows, err := r.db.Query(`
		SELECT id, name, server_id, domain, port, compose,
		       git_repo, git_branch, status, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&gitRepo, &gitBranch, &a.Status, &a.CreatedAt, &a.UpdatedAt,
		); err != nil {
			return nil, err
		}
...
func (r *Repository) Create(a *App) error {
	result, err := r.db.Exec(`
		INSERT INTO apps (name, server_id, domain, port, compose, git_repo, git_branch, status)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, a.Name, a.ServerID, a.Domain, a.Port, a.Compose, nullString(a.GitRepo), nullString(a.GitBranch), "created")
	if err != nil {
...
func (r *Repository) Update(a *App) error {
	_, err := r.db.Exec(`
		UPDATE apps SET
			...
			git_repo=?, git_branch=?, status=?, updated_at=CURRENT_TIMESTAMP
		WHERE id=?
	`, a.Name, a.ServerID, a.Domain, a.Port, a.Compose,
		nullString(a.GitRepo), nullString(a.GitBranch), a.Status, a.ID)
	return err
}
